...
export class AddUserActivityPage {
  ...
  divisor(a){
    ...
    pareja=  Number(a.d_pareja.slice(11,13))+Number(a.d_pareja.slice(14,16));
      ...
      this.userActivityAddList$.subscribe(
        userActivities => {
          if(userActivities.length > 0){
            ...
            //let diference =0
            //if(totalHours < 24){
              //let diference = (24-Number(this.userActivity.d_suenho_descanso))/7
  ...
  sumaactividad(actividad_,suma,div,cp){
    ...
    {
      if(div==6) // pareja es mayor que 0
      {
        nuevahora=(horaActividad+minutoactividad)+(suma/div);
        nhora=Math.trunc(nuevahora);
        nminuto = Number((nuevahora-nhora).toFixed(2));
    ...
    actividad_ = actividad_.slice(0,11)+'0'+tiempoformateado;
    return actividad_;
  }
